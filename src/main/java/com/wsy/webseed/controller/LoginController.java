...
public class LoginController {
    ...
    public ResponseEntity<String> login(HttpServletRequest request){
        ...
        try {
            ...
            if (currentUser.isAuthenticated()) {
                ...
                LOGGER.info("OPERATION_LOG:[{}][login]User {} login success.", username, password);
            } else {
    ...
    public ResponseEntity<String> editPwd(@RequestBody final String json,HttpServletRequest request){
        final HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType("text","plain",Charset.forName("utf-8")));
        String result = "";
        String loginName = StringUtils.defaultIfEmpty((String) request.getSession().getAttribute(SysConstant.SESSION_LOGIN_ID_KEY),"");
